...
document.addEventListener('DOMContentLoaded', function () {
    // COMBINA OS TRÊS CONJUNTOS DE REQUISITOS
    const todosRequisitos = Object.values(cruzamentoRequisitos);
    const requisitosVerificar = Object.values(cruzamentoRequisitos2);
    const requisitosFaltantes = Object.values(cruzamentoRequisitos3);
    const todos = [...todosRequisitos, ...requisitosVerificar, ...requisitosFaltantes];
    ...
    // === ORDENA OS REQUISITOS POR ID ===
    requisitosAtuais = Object.values(cruzamentoRequisitosCompleto).sort((a, b) => {
...
function renderPendencias() {
    ...
    // === ORDENA POR ID ===
    pendentes.sort((a, b) => a.id.localeCompare(b.id, undefined, { numeric: true, sensitivity: 'base' }));
...
function renderRequisitosSecao(capituloKey, secaoKey) {
    ...
    let requisitosFiltrados = todosRequisitos.filter(r => r.secao === secaoKey);
    ...
    // === ORDENA POR ID ===
    requisitosFiltrados.sort((a, b) => a.id.localeCompare(b.id, undefined, { numeric: true, sensitivity: 'base' }));
...
function abrirModal(id) {
    ...
    // Função auxiliar para ordenar IDs
    function ordenarIds(ids) {
    ...
    if (linhas.length > 0) {
        ...
        // Ordena os IDs visíveis
        listaIds = ordenarIds(listaIds);
    } else {
        listaIds = ordenarIds(Object.keys(dados));
    }
